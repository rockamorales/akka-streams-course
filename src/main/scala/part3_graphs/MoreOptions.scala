...
object MoreOptions extends App {
...
//  max3RunnableGraph.run()
  ...
  val transactionSource = Source(
    List(
      Transaction("5253525535352", "Paul", "Jim", 100, new Date),
      Transaction("5253525535353", "Daniel", "Jim", 100000, new Date),
      Transaction("5253525535354", "", "Jim", 7000, new Date)
    )
  )
  ...
  val supiciusTxnStaticGraph = GraphDSL.create(){ implicit builder =>
    import GraphDSL.Implicits._
  ...
  val suspiciousTxnRunnableGraph = RunnableGraph.fromGraph(
    GraphDSL.create(){ implicit builder =>
      ...
      // step 2
      val suspiciousTxnShape = builder.add(supiciusTxnStaticGraph)

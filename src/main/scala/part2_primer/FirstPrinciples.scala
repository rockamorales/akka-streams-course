...
object FirstPrinciples extends App {
  ...
  // every akka streams starts with a source
  val source = Source(1 to 10)
  // every akka streams ends with a sink
...
//  illegalSource.to(Sink.foreach(println)).run()
  ...
  // Formal way to construct a stream is
...
*/
  ...
  /**
   * Exercise: create a stream that takes the names of persons, the you will keep the first 2 names with length > 5 characters
   *
   */

  val persons = List("Roberto", "Jorge", "Any", "Violeta", "Jose", "Juan", "Joe", "Carlos", "Marcos")
  val nameSource = Source(persons);
...
//    .via(Flow[String]
//      .take(2))
//    .to(Sink.foreach[String](println)).run()
